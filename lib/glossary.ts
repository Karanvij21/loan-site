...
export const glossaryTerms: GlossaryTerm[] = [
  ...
  },
  {
    ...
    aka: ["amortization table", "loan payment schedule"],
    category: "Rates & terms",
    short: "A table showing each scheduled loan payment broken down into principal and interest components, along with the remaining balance after each payment. Every personal loan has an amortization schedule; most lenders provide one at origination or make it available in the online account portal.",
    long: "An amortization schedule is the full payment-by-payment breakdown of a loan's lifecycle. It answers the question: 'Of my $350 monthly payment, how much goes to principal vs interest each month?'\n\nHow to read one: Each row represents one payment period (month). Columns typically show: Payment number, Payment date, Payment amount, Interest paid this period, Principal paid this period, Remaining balance. In early periods, the interest column is high (the balance is large, so interest accrues faster). In later periods, the principal column is high (the balance is small, so more of each payment reduces the balance).\n\nExample: A $10,000 loan at 12% APR over 36 months has a $332.14 monthly payment. Month 1: $100 interest ($10,000 x 0.12/12), $232.14 principal, balance $9,767.86. Month 36: ~$3.29 interest, ~$328.85 principal, balance $0. The interest component is 30x larger in month 1 than in month 36, even though the payment amount is identical throughout.\n\nWhy it matters: Looking at the amortization schedule before signing shows you the exact total interest cost over the loan life. It also shows the outstanding balance at any point in the future, which you can use to plan a prepayment strategy - for example, making an extra $500 payment in month 6 reduces every subsequent month's interest by eliminating $500 of principal that would have accrued interest for the remaining 30 months.\n\nGenerating your own: Online amortization calculators (Bankrate, NerdWallet) generate full schedules for any loan amount, rate, and term. Compare the total interest across different loan terms before choosing.",
    related: ["amortization", "fully-amortizing", "prepayment-penalty", "principal"],
  },
  {
    slug: "escrow",
    name: "Escrow",
    aka: ["escrow account", "impound account"],
    category: "Application",
    short: "A neutral third-party account that holds funds during a transaction until specified conditions are met. In mortgage lending, lenders often require an escrow account for tax and insurance payments. Personal loans do not typically involve escrow accounts - funds are disbursed directly to the borrower.",
    long: "Escrow is a financial arrangement where a third party holds funds or assets until the terms of a contract are fulfilled. In consumer lending and real estate, escrow serves different functions:\n\nMortgage escrow accounts: When you have a mortgage, the lender often requires an escrow account for property taxes and homeowners insurance. Each month, a portion of your mortgage payment goes into the escrow account. The lender uses those funds to pay your property taxes and insurance premiums when they come due. This protects the lender by ensuring taxes and insurance are paid (which protects the collateral).\n\nReal estate purchase escrow: When you buy a home, your earnest money deposit and eventually the full purchase price are held in escrow by a title company or escrow company until closing. The funds are released to the seller when all conditions are met (inspections passed, title cleared, loan funded).\n\nPersonal loans and escrow: Standard personal loans do not involve escrow. When you get a personal loan, the lender disburses funds directly to your bank account (for general purposes) or sometimes directly to a creditor (for debt consolidation). There is no third-party escrow account holding the funds.\n\nException - contractor payment escrow: A few home improvement personal loan programs hold funds in escrow and release them to contractors upon completion of work milestones. This protects the borrower from paying in full upfront before work is complete. These programs are less common than standard personal loans but available from some lenders for renovation projects.",
    related: ["disbursement", "loan-proceeds", "home-equity-loan"],
  },
  {
    slug: "floor-rate",
    name: "Floor Rate",
    aka: ["interest rate floor", "minimum rate"],
    category: "Rates & terms",
    short: "The minimum interest rate a variable-rate loan can fall to, regardless of how low the underlying index rate drops. If the Prime Rate falls below the floor rate, the borrower's rate stays at the floor. Floor rates protect lenders from negative or near-zero interest margins.",
    long: "A floor rate is a contractual minimum rate on a variable-rate loan. It prevents the borrower's interest rate from falling below a specified level even if the index rate (Prime Rate, SOFR, etc.) drops significantly.\n\nHow it works: A variable-rate personal loan with a rate of Prime + 5% and a 6% floor rate: if Prime is 7.50%, rate = 12.50%. If Prime falls to 4%, rate would normally be 9%, but the floor prevents it from going below 6%, so the rate stays at 6% even though Prime + 5% would be 9%.\n\nWait - in the example above, 9% is above the floor of 6%, so the floor is not triggered. A floor only matters when the index plus margin calculation falls below the floor. Example where floor matters: floor rate of 8%, Prime falls to 2%, Prime + 3% margin = 5%, which is below the 8% floor, so rate stays at 8%.\n\nWhen floors appear in personal lending: Floor rates are more common in commercial and mortgage lending than in consumer personal loans. Variable-rate personal loans (already uncommon) sometimes include a floor to protect the lender from extremely low rate environments. In the 2020-2021 near-zero rate environment, some lenders' variable rate products hit their floor rates.\n\nBorrower implication: A floor rate limits the downside protection of variable-rate borrowing. One of the benefits of variable rates is that they decline when central banks cut rates. A floor caps that benefit. Check for a floor rate when evaluating any variable-rate personal loan.",
    related: ["variable-rate", "rate-cap", "prime-rate", "index-rate"],
  },
  {
    slug: "balloon-loan",
    name: "Balloon Loan",
    aka: ["bullet loan", "term loan with balloon"],
    category: "Rates & terms",
    short: "A loan that requires a large lump-sum payment (the 'balloon') at the end of the term, after a period of smaller regular payments. Standard personal loans are NOT balloon loans - they are fully amortizing, meaning the final regular payment brings the balance to zero. Balloon structures appear in some commercial and real estate lending.",
    long: "A balloon loan's regular payments (typically monthly) do not fully amortize the principal over the loan term. After the final regular payment, a large remaining balance (the 'balloon') is due in full. This creates significant risk for borrowers who cannot pay the balloon or refinance.\n\nExample: A 5-year balloon loan for $100,000 at 8% might have monthly payments calculated as if it were a 30-year mortgage ($733/month). After 5 years, the borrower has made 60 payments but the balance is still approximately $90,000, which is all due as a balloon payment. The borrower must pay $90,000 cash, refinance, or sell the asset.\n\nWhere balloon loans appear: Commercial real estate mortgages (5/25 or 7/20 balloon/amortization structures). Some adjustable-rate mortgages. Commercial equipment and vehicle financing. Agricultural loans tied to harvest cycles. Seller-financed real estate transactions.\n\nPersonal loans and balloons: Standard consumer personal loans are NOT balloon loans. A 36-month personal loan has equal monthly payments that fully pay off the principal and interest, with no balloon due at month 36. If you see a personal loan that appears to have a large final payment, read the terms carefully - it may indicate a product that is not a standard amortizing personal loan.\n\nRisk of balloon structures: If you cannot pay the balloon when due, you must refinance (which depends on your credit and market conditions at that future date) or default. This uncertainty is why most consumer protection advocates prefer standard amortizing loans for personal borrowing needs.",
    related: ["fully-amortizing", "amortization", "maturity-date", "refinance"],
  },
  {
    slug: "credit-mix",
    name: "Credit Mix",
    aka: ["account mix", "types of credit"],
    category: "Credit",
    short: "One of the five factors that make up your FICO credit score, accounting for approximately 10% of the score. Credit mix refers to the variety of credit types you have: revolving accounts (credit cards, lines of credit) and installment accounts (personal loans, auto loans, mortgages). Having both types can positively affect your score.",
    long: "Credit mix is the smallest weighted factor in the FICO scoring model (10%), but it is the one factor most directly affected by taking out a personal loan.\n\nWhy credit mix matters: Lenders like to see that borrowers can responsibly manage different types of credit. Someone who has only credit cards has demonstrated ability to manage revolving credit but not installment debt. Adding a personal loan (or vice versa) demonstrates a broader credit management capability.\n\nHow a personal loan affects credit mix: If you currently have only credit cards (revolving accounts), adding a personal loan (installment account) adds to your credit mix. This typically produces a small positive effect on your FICO score over time. However, the 10% weight means it is a minor factor. A higher credit score is primarily driven by payment history (35%), amounts owed (30%), and length of credit history (15%).\n\nBorrowing just for credit mix: Taking out a personal loan solely to improve credit mix is almost never worth it. The interest cost of a personal loan far exceeds any score benefit from marginally improved credit mix. Credit-builder loans (designed specifically for this purpose) cost much less than standard personal loans and achieve the same score diversification benefit.\n\nThe five FICO factors: (1) Payment history - 35%. (2) Amounts owed / utilization - 30%. (3) Length of credit history - 15%. (4) New credit / hard inquiries - 10%. (5) Credit mix - 10%. Improving the top two factors has the most impact on your score.",
    related: ["credit-score", "fico-score", "installment-loan", "revolving-credit", "credit-builder-loan"],
  },
  {
    slug: "net-worth",
    name: "Net Worth",
    aka: ["personal net worth", "wealth"],
    category: "Application",
    short: "The total value of all your assets minus all your liabilities. Net worth is not typically required for personal loan applications, but it can be a factor in manual underwriting for large loan amounts. A positive net worth demonstrates financial stability beyond income and credit score.",
    long: "Net worth = total assets - total liabilities. Assets include: checking and savings account balances, retirement accounts (401k, IRA), investment accounts, real estate equity (home value minus mortgage balance), vehicles, and other valuable property. Liabilities include: mortgage balance, auto loans, student loans, credit card balances, personal loan balances, and any other debts.\n\nNet worth vs income for lending purposes: Most personal loan underwriting is income-and-credit-score based. Net worth is rarely collected or considered by algorithmic underwriting at online lenders. However, at banks and credit unions with manual underwriting (especially for large loans of $50,000+), loan officers consider assets as evidence of financial stability and the ability to cover payments if income temporarily stops.\n\nHow to calculate your net worth: List all assets with current market values. List all debts with current balances. Subtract total debts from total assets. A positive net worth means assets exceed debts. A negative net worth (more debt than assets) is common for young borrowers with student loans and little equity.\n\nWhy net worth matters beyond lending: Net worth is the most comprehensive measure of financial health. A person earning $100,000/year with $300,000 in debt has lower financial security than someone earning $60,000 with $0 in debt. Building net worth - through debt repayment, savings, and investment - is the long-term financial goal that personal loans can either serve (by funding investments or debt consolidation that improves net worth) or undermine (by funding consumption that does not increase assets).\n\nImpact of a personal loan on net worth: A personal loan used for consumption (vacation, entertainment) temporarily reduces net worth (you now have a liability without a corresponding asset). A personal loan used to fund education or a business may increase future earning power and ultimately net worth.",
    related: ["gross-income", "net-income", "debt-to-income-ratio", "creditworthiness"],
  },
  {
    slug: "introductory-rate",
    name: "Introductory Rate",
    aka: ["teaser rate", "promotional rate", "intro APR"],
    category: "Rates & terms",
    short: "A temporarily reduced interest rate offered for a limited period (typically 6-21 months) to attract new borrowers. Most commonly seen on credit cards, but some personal loan lenders and personal line of credit products offer introductory rates. After the intro period, the rate typically resets to the standard (higher) rate.",
    long: "An introductory rate is a marketing tool that sets a reduced initial rate to attract new accounts. The true long-term cost depends on the standard rate that applies after the intro period ends.\n\nWhere you see introductory rates: Credit cards: 0% APR for 15-21 months on purchases or balance transfers is the most common introductory rate in consumer lending. Personal lines of credit (PLOCs): some lenders offer 0% or low introductory rates for the first 6-12 months. Personal loans: less common for installment loans, but some platforms offer first-loan rates that are below their standard pricing.\n\nThe difference between deferred interest and 0% APR: True 0% APR: you pay no interest during the promotional period. If balance remains at the end, you just start paying the standard rate on that remaining balance going forward. No retroactive interest. Deferred interest (common at retail stores): interest accrues throughout the promotional period but is 'deferred' (not charged yet). If you pay the full balance before the deadline, the accumulated interest is waived. If any balance remains, you are charged all the accumulated interest on the original balance retroactively - often 26.99%-29.99% APR on the full original amount, not just the remaining balance. This distinction is critical: deferred interest is extremely punishing; true 0% APR is not.\n\nFor personal loans specifically: Standard personal loans (fixed-term, fixed-rate) do not typically have introductory rates - the rate you get at origination is your rate for the life of the loan. Introductory rates are a revolving credit (credit card, line of credit) feature. If a lender offers you a personal loan with an introductory period, read the terms carefully to understand what rate applies after the intro period and how it is structured.",
    related: ["apr", "promotional-rate", "deferred-interest", "variable-rate"],
  },
  {
    slug: "piggyback-loan",
    name: "Piggyback Loan",
    aka: ["80-10-10 loan", "combo loan", "second mortgage"],
    category: "Rates & terms",
    short: "A second loan taken simultaneously with a primary mortgage to avoid PMI or cover part of the down payment. The most common structure is an 80-10-10: 80% first mortgage, 10% second mortgage (often a HELOC), and 10% down payment. Not directly a personal loan product, but sometimes confused with one.",
    long: "A piggyback loan involves two mortgage loans originated simultaneously: a primary first mortgage (usually 80% of the purchase price) and a second mortgage (10%-20% of the purchase price). This combination avoids private mortgage insurance (PMI), which is required when a borrower puts less than 20% down on a conventional mortgage.\n\nCommon piggyback structures: 80-10-10: 80% first mortgage, 10% second mortgage (often a HELOC or home equity loan), 10% buyer down payment. Total LTV = 90%, but the primary mortgage is at 80% so no PMI. 80-15-5: 80% first mortgage, 15% second, 5% down. Less common now but historically popular.\n\nWhy borrowers use piggybacks: PMI on a conventional loan costs 0.5%-1.5% of the loan amount annually (for example, $1,000-$3,000/year on a $200,000 loan). A second mortgage at 7%-10% APR may be less expensive annually than PMI, especially if the second mortgage has a shorter term or can be paid off faster.\n\nHow it relates to personal loans: Piggyback loans are mortgage products, not personal loans. However, borrowers sometimes confuse the two when considering home purchase financing options. A personal loan cannot be used as a down payment in most cases: conventional mortgages require that the down payment come from the borrower's own funds (not borrowed), and lenders verify the source of down payment funds. Using a personal loan as a down payment typically violates loan terms and is discoverable through credit report and bank statement review.\n\nCurrent availability: Piggyback loans are less common post-2008 than they were in the early 2000s, but they are available from portfolio lenders, credit unions, and some banks for qualified borrowers.",
    related: ["heloc", "home-equity-loan", "lien"],
  },
  {
    slug: "capacity",
    name: "Capacity (Lending)",
    aka: ["repayment capacity", "ability to repay"],
    category: "Application",
    short: "One of the 5 Cs of credit - a borrower's financial ability to repay a loan based on current income, expenses, and existing debt obligations. Lenders measure capacity primarily through the debt-to-income (DTI) ratio. High capacity (low DTI, high income) leads to better loan terms.",
    long: "Capacity answers the lender's core question: can this borrower actually afford to make the monthly payments? Unlike credit history (which shows what the borrower has done in the past), capacity looks at whether the borrower's current financial situation supports adding a new debt payment.\n\nHow capacity is measured: Debt-to-income ratio (DTI): the primary capacity metric. Total monthly debt obligations divided by gross monthly income. A 10% DTI (monthly payments = 10% of income) represents high capacity. A 50% DTI represents very low capacity. Most personal loan lenders require DTI below 40%; many prefer below 35%. Residual income: income remaining after all debts and essential expenses. Higher residual income means more buffer if income dips. Employment stability: a salaried W-2 employee has more predictable income than a self-employed contractor. Lenders assign higher capacity confidence to stable employment.\n\nHow to increase your capacity before applying: Pay off small debts to reduce monthly obligations. Increase income (and document it: pay raise, second job, rental income). Time the application to avoid periods of low income (seasonal workers apply during peak season). Include all income sources - alimony, social security, investment income, rental income all count under ECOA.\n\nCapacity vs creditworthiness: Credit score measures past behavior; capacity measures present ability. A borrower with a 780 FICO score but 55% DTI (income barely covers existing debts) has excellent creditworthiness but poor capacity. Most lenders consider both - a high credit score cannot fully compensate for truly insufficient income.",
    related: ["debt-to-income-ratio", "creditworthiness", "gross-income", "net-monthly-income"],
  },
  {
    slug: "right-of-rescission",
    name: "Right of Rescission",
    aka: ["cooling-off period", "right to cancel"],
    category: "Regulation",
    short: "A federally mandated right under the Truth in Lending Act (TILA) for borrowers to cancel certain credit transactions within 3 business days without penalty. The right of rescission applies primarily to home-secured loans (HELOCs, cash-out refinances) - NOT to standard unsecured personal loans.",
    long: "The right of rescission gives borrowers a 3-business-day window to cancel a loan after signing the credit agreement, without penalty or obligation. This right is codified under Regulation Z of TILA (15 U.S.C. 1635).\n\nWhat the right of rescission covers: Home-secured credit transactions where the borrower's primary residence is the collateral. This includes: HELOCs, home equity loans, cash-out refinances on a primary residence. It does NOT cover: purchase money mortgages (the original mortgage to buy the home), refinances that do not change the amount of the lien, or investment property transactions.\n\nWhat the right of rescission does NOT cover: Unsecured personal loans - no right of rescission under TILA. The right of rescission is specifically for home-secured credit to protect borrowers from losing their homes impulsively.\n\nHow the 3 days are counted: Business days for rescission purposes include Saturdays but not Sundays or federal public holidays. Day 0 is the day you sign the credit agreement. You have until midnight of the 3rd business day to rescind. If you sign on Friday, the rescission period expires at midnight the following Tuesday (Saturday counts, Sunday does not).\n\nHow to exercise the right: Send written notice (mail, fax, email if lender permits) to the lender within the 3-day window. The lender must return any money you paid within 20 calendar days of receiving the rescission notice.\n\nState law analogs: Some states have additional cooling-off periods or cancellation rights for certain consumer transactions (door-to-door sales, gym memberships, home improvement contracts). These vary by state and are separate from the federal TILA right of rescission.",
    related: ["tila", "heloc", "home-equity-loan", "loan-disclosure"],
  },
];
